    ...
    <div className="space-y-3">
      {lessons.map((lesson) => (
        <Link
          key={lesson.id}
          href={lesson.href}
          className="card-surface flex flex-col gap-4 rounded-[22px] p-4 transition hover:-translate-y-0.5 hover:border-[var(--brand)] hover:shadow-[0_18px_44px_rgba(35,27,18,0.1)] lg:flex-row lg:items-center lg:justify-between"
        >
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm font-semibold text-[var(--brand-deep)]">Open les</span>
          </div>
        </Link>
      ))}
    </div>
